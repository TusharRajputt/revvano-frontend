...
import { Upload, Trash2, Play, Plus, Minus } from 'lucide-react';
...
export function AdminSettingsPage() {
  ...
  if (settings && !initialized) {
    ...
    setSizeGuideNote(settings.sizeGuide.note);
    setSizeGuideHeaders(settings.sizeGuide.headers);
    setSizeGuideRows(settings.sizeGuide.rows);
    setInitialized(true);
  }
  ...
  const handleSaveSizeGuide = async () => {
    ...
    try {
      const updated = await settingsService.updateSizeGuide({
        note: sizeGuideNote,
        headers: sizeGuideHeaders,
        rows: sizeGuideRows,
      });
      refreshSettings(updated);
      toast({ title: 'Size guide updated' });
    } catch {
      toast({ title: 'Could not update size guide', variant: 'destructive' });
    } finally {
  ...
  const updateHeader = (index: number, value: string) => {
    setSizeGuideHeaders((prev) => prev.map((h, i) => (i === index ? value : h)));
  };

  const updateCell = (rowIndex: number, colIndex: number, value: string) => {
    setSizeGuideRows((prev) =>
      prev.map((row, i) => (i === rowIndex ? row.map((c, j) => (j === colIndex ? value : c)) : row))
    );
  };
  ...
  const removeColumn = (index: number) => {
    ...
    setSizeGuideHeaders((prev) => prev.filter((_, i) => i !== index));
    setSizeGuideRows((prev) => prev.map((row) => row.filter((_, i) => i !== index)));
  };

  const addRow = () => {
    setSizeGuideRows((prev) => [...prev, sizeGuideHeaders.map(() => '')]);
  };
  ...
  return (
    <div className="max-w-3xl space-y-10">
      ...
      {/* Size Guide */}
      <section className="border border-border p-6">
        <h2 className="font-medium mb-1">Size Guide</h2>
        <p className="text-xs text-muted-foreground mb-4">
          Shown to customers when they click "Size Guide" on any product page.
        </p>

        <label className="block text-xs font-semibold uppercase tracking-widest mb-1.5">Note</label>
        <textarea
          value={sizeGuideNote}
          onChange={(e) => setSizeGuideNote(e.target.value)}
          rows={2}
          className="w-full border border-border px-3 py-2 text-sm focus:outline-none focus:border-accent mb-4"
        />
          ...
          <table className="w-full text-sm border-collapse">
            <thead>
              <tr>
                ...
                <th className="p-1">
                  <button onClick={addColumn} className="p-1 text-muted-foreground hover:text-accent" title="Add column">
            ...
            <tbody>
              {sizeGuideRows.map((row, ri) => (
                <tr key={ri}>
                  ...
                  <td className="p-1">
                    <button onClick={() => removeRow(ri)} className="p-1 text-muted-foreground hover:text-destructive">
        ...
        <div className="flex items-center gap-3">
          ...
          <Button onClick={handleSaveSizeGuide} disabled={savingSizeGuide}>
