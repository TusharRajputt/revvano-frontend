...
export function AdminProductFormPage() {
  ...
  const [variantColorHex, setVariantColorHex] = useState('#cccccc');
  const [variantSize, setVariantSize] = useState<ProductVariant['size']>('M');
  const [variantStock, setVariantStock] = useState('0');
  const [variantSku, setVariantSku] = useState('');
  const [tags, setTags] = useState<string[]>([]);
  const [tagInput, setTagInput] = useState('');
  const [useCustomSizeGuide, setUseCustomSizeGuide] = useState(false);
  const [sizeGuideNote, setSizeGuideNote] = useState('');
  const [sizeGuideHeaders, setSizeGuideHeaders] = useState<string[]>(['Size', 'Bust', 'Waist', 'Hips']);
  const [sizeGuideRows, setSizeGuideRows] = useState<string[][]>([['M', '', '', '']]);
      ...
      .then(([cats, cols, product]) => {
        ...
        if (product) {
          setForm({
            ...
            category: product.categoryId || cats.find((c) => c.name === product.category)?.id || '',
          ...
          setVariants(product.variants && product.variants.length ? product.variants : []);
          setUseCustomSizeGuide(!!product.sizeGuide);
          if (product.sizeGuide) {
            setSizeGuideNote(product.sizeGuide.note);
            setSizeGuideHeaders(product.sizeGuide.headers);
            setSizeGuideRows(product.sizeGuide.rows);
          }
  ...
  const addVariant = () => {
    ...
    setVariants((prev) => [
      ...prev,
      { color: variantColor.trim(), colorHex: variantColorHex, size: variantSize, stock: Number(variantStock) || 0, sku: variantSku.trim() },
    ]);
    setVariantColor('');
    setVariantColorHex('#cccccc');
    setVariantStock('0');
    setVariantSku('');
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        tags,
        variants,
        sizeGuide: useCustomSizeGuide ? { note: sizeGuideNote, headers: sizeGuideHeaders, rows: sizeGuideRows } : undefined,
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <section className="rounded-lg border border-border bg-background p-6 space-y-4">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5">
              ...
              <p className="text-xs text-muted-foreground">
                Also sets which "Shop by Mood" section this product appears under.
              </p>
            </div>
            <div className="space-y-1.5">
        ...
        <section className="border border-border rounded-lg p-6 space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="font-medium">Custom Size Guide</h2>
              <p className="text-xs text-muted-foreground">
                Off by default (uses the site's global size guide). Enable to set a size chart specific to this product.
              </p>
            </div>
            <Switch checked={useCustomSizeGuide} onCheckedChange={setUseCustomSizeGuide} />
          </div>
          ...
          {useCustomSizeGuide && (
            <>
                    ...
                    <tr>
                        ...
                        <th key={i} className="p-1">
                          <div className="flex items-center gap-1">
                            <input
                              value={h}
                              onChange={(e) =>
                                setSizeGuideHeaders((prev) => prev.map((x, j) => (j === i ? e.target.value : x)))
                              }
                              className="w-full border border-border px-2 py-1 text-xs font-semibold uppercase"
                            />
                            <button
                              type="button"
                              onClick={() => {
                                if (sizeGuideHeaders.length <= 1) return;
                                setSizeGuideHeaders((prev) => prev.filter((_, j) => j !== i));
                                setSizeGuideRows((prev) => prev.map((row) => row.filter((_, j) => j !== i)));
                              }}
                              disabled={sizeGuideHeaders.length <= 1}
                      ...
                      <th className="p-1">
                        <button
                          type="button"
                          onClick={() => {
                            setSizeGuideHeaders((prev) => [...prev, 'New Column']);
                            setSizeGuideRows((prev) => prev.map((row) => [...row, '']));
                          }}
                    ...
                    {sizeGuideRows.map((row, ri) => (
                      <tr key={ri}>
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                setSizeGuideRows((prev) =>
                                  prev.map((r, i) => (i === ri ? r.map((c, j) => (j === ci ? e.target.value : c)) : r))
                                )
                              }
                        ...
                        <td className="p-1">
                          <button
                            type="button"
                            onClick={() => setSizeGuideRows((prev) => prev.filter((_, i) => i !== ri))}
              ...
              <button
                type="button"
                onClick={() => setSizeGuideRows((prev) => [...prev, sizeGuideHeaders.map(() => '')])}
